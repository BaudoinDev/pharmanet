<aside class="sidebar">
  ...
  <nav class="sidebar-nav">
    <div style="font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#9ca3af;padding:.5rem .85rem .35rem;">Espace pharmacie</div>
    <ul class="list-unstyled mb-0">
      <li>
        <a href="{{ route('pharmacie.dashboard') }}" class="nav-link {{ ($active ?? '') === 'dashboard' ? 'active' : '' }}">
          <i class="ti ti-layout-dashboard"></i> Tableau de bord
        </a>
      </li>
      <li>
        <a href="{{ route('pharmacie.commandes') }}" class="nav-link {{ ($active ?? '') === 'commandes' ? 'active' : '' }}">
          <i class="ti ti-shopping-bag"></i> Commandes
        </a>
      </li>
      <li>
        <a href="{{ route('pharmacie.ordonnances') }}" class="nav-link {{ ($active ?? '') === 'ordonnances' ? 'active' : '' }}">
          <i class="ti ti-file-description"></i> Ordonnances
        </a>
      </li>
      <li>
        <a href="{{ route('pharmacie.medicaments') }}" class="nav-link {{ ($active ?? '') === 'medicaments' ? 'active' : '' }}">
          <i class="ti ti-pill"></i> Médicaments
        </a>
      </li>
      <li>
        <a href="{{ route('pharmacie.categories') }}" class="nav-link {{ ($active ?? '') === 'categories' ? 'active' : '' }}">
          <i class="ti ti-tag"></i> Catégories
        </a>
      </li>
      <li>
        <a href="{{ route('pharmacie.parametres') }}" class="nav-link {{ ($active ?? '') === 'parametres' ? 'active' : '' }}">
  ...
  <div class="sidebar-footer">
    <div class="d-flex align-items-center gap-2 px-2 mb-1">
      <div class="avatar-initials">
        {{ strtoupper(substr($pharmacie->nom ?? 'PH', 0, 2)) }}
      </div>
      <div style="min-width:0;">
        <div class="fw-semibold text-truncate" style="font-size:.8rem;">{{ $pharmacie->nom ?? '' }}</div>
